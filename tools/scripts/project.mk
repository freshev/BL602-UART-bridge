...
# This Makefile is included directly from the user project Makefile in order to call the Makefile.mk
...
#
# This makefile requires the environment variable PROJECT_SDK_PATH to be set to the top-level bl60x_sdk directory
...
# make PROJECT_SDK_PATH a "real" absolute path
...
SANITISED_PROJECT_SDK_PATH:=$(realpath $(PROJECT_SDK_PATH))
else
SANITISED_PROJECT_SDK_PATH:=$(realpath $(wildcard $(PROJECT_SDK_PATH)))
endif

export PROJECT_SDK_PATH := $(SANITISED_PROJECT_SDK_PATH)

ifndef PROJECT_SDK_PATH
$(error PROJECT_SDK_PATH variable is not set to a valid directory.)
endif

ifneq ("$(PROJECT_SDK_PATH)","$(SANITISED_PROJECT_SDK_PATH)")
$(error If PROJECT_SDK_PATH is overriden on command line, it must be an absolute path with no embedded shell special characters)
endif

ifneq ("$(PROJECT_SDK_PATH)","$(subst :,,$(PROJECT_SDK_PATH))")
$(error PROJECT_SDK_PATH cannot contain colons. If overriding PROJECT_SDK_PATH on Windows, use MSYS Unix-style /c/dir instead of C:/dir)
endif
...
COMMON_MAKEFILES := $(abspath $(PROJECT_SDK_PATH)/tools/scripts/project.mk $(PROJECT_SDK_PATH)/tools/scripts/common.mk $(PROJECT_SDK_PATH)/tools/scripts/component_wrapper.mk $(firstword $(MAKEFILE_LIST)))
export COMMON_MAKEFILES

# The directory where we put all objects/libraries/binaries. The project Makefile can
# configure this if needed.
ifndef BUILD_DIR_BASE
BUILD_DIR_BASE := $(PROJECT_PATH)/out
endif
...
COMPONENT_DIRS := $(PROJECT_PATH)/sdk/$(SDK_VERSION) $(PROJECT_PATH)/src $(PROJECT_PATH)/example $(PROJECT_PATH)/thirdpartylib
endif
...
include $(PROJECT_SDK_PATH)/components.mk
...
include $(PROJECT_SDK_PATH)/tools/scripts/common.mk
include Makefile.mk
...
COMMON_WARNING_FLAGS = -Wall -Werror=all \
	...
	-Wno-error=restrict -Wno-error=format= -Wno-error=implicit-function-declaration
...
# stored in COMPONENT_LINKER_DEPS, built via Makefile.mk files' COMPONENT_ADD_LINKER_DEPS variable
...
+$(MAKE) -C $(BUILD_DIR_BASE)/$(2) -f $(PROJECT_SDK_PATH)/tools/scripts/component_wrapper.mk COMPONENT_MAKEFILE=$(1)/Makefile.mk COMPONENT_NAME=$(2)
endef
...
#
$(BUILD_DIR_BASE)/$(2)/component_project_vars.mk: $(1)/Makefile.mk $(COMMON_MAKEFILES) $(SDKCONFIG_MAKEFILE) | $(BUILD_DIR_BASE)/$(2)
