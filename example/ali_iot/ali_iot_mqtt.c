...
static void ali_mqtt_server_addr_init(void)
{
    ...
    sprintf(ali_mqtt_server_addr,"%s.iot-as-mqtt.%s.aliyuncs.com",IOT_PRODUCTKEY,AliMqttConnectPara.server_region);
...
{   
    ...
    while(1)
    {
        ...
        u_printf("======>connect server:%s\n",ALI_MQTT_SERVER_ADDR);
...
void get_ali_mqtt_connect_para(void)
{
    ...
    if((AliMqttConnectPara.head!=ALI_MQTT_CONNECT_PARA_HEAD)||(AliMqttConnectPara.tail!=ALI_MQTT_CONNECT_PARA_TAIL))
    {
        ...
        strcpy(AliMqttConnectPara.server_region,"cn-shanghai");
...
int Ali_MQTT_thread_start(void)
{
    ...
    if(hfthread_create(Ali_MQTTClient_thread, "Ali_MQTTClient_thread", 2048, NULL, HFTHREAD_PRIORITIES_LOW, NULL, NULL)!= HF_SUCCESS)
	{
