		...
		case HFE_BLE_DMS_OK:
			{
				...
				if(userdata == NULL)
					u_printf("userdata is null!\n");
				else
				{
					int i;
					for(i=0; i<(int)strlen(userdata); i++)
					{
...
static int hf_set_wap(int8_t wap_mode,int8_t wap_channel,int8_t ap_enable_hide,char *ap_ssid)
{
	memset(g_hf_config_file.ap_ssid, 0, sizeof(g_hf_config_file.ap_ssid));
	strcpy((char *)g_hf_config_file.ap_ssid, ap_ssid);
	//g_hf_config_file.ap_pmk_av= 0;
	g_hf_config_file.wap_mode=wap_mode;
	g_hf_config_file.wap_channel=wap_channel;
	g_hf_config_file.ap_enable_hide=ap_enable_hide;
	return 0;
}
static int hf_set_sta(char *ssid,char *key,enum ENWAUTH wsauth,enum ENWENCRY wsencry)
{
	memset(g_hf_config_file.sta_ssid, 0, sizeof(g_hf_config_file.sta_ssid));
	strcpy((char *)g_hf_config_file.sta_ssid, ssid);
	bzero(g_hf_config_file.sta_key,sizeof(g_hf_config_file.sta_key));
	strcpy((char *)(g_hf_config_file.sta_key), key);
	g_hf_config_file.wsauth = wsauth;
	g_hf_config_file.wsencry = wsencry;
...
{	
	...
	while(1)
	{
		...
		if(g_hf_config_file.wmode_mode == CONFIG_WMODE_STA)
		{
			u_printf("switch to ap!\r\n");
			g_hf_config_file.wmode_mode=CONFIG_WMODE_AP;
			//hf_set_wap(CONFIG_WAMOD_11BGN,0,0,"hfwjt"); //start new ap
		}
		else if(g_hf_config_file.wmode_mode == CONFIG_WMODE_AP)
		{
			u_printf("switch to sta!\r\n");
			g_hf_config_file.wmode_mode=CONFIG_WMODE_STA;
			//hf_set_sta("HF-HUAWEI","12345678",WAUTH_WPA2PSK,WENCRY_AES); //connect to ap
		}
		hfwifi_start();
	}
