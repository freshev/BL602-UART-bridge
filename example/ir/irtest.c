...
static void ir_recv_callback(uint32_t event,unsigned int data,uint32_t len,uint32_t buf_len)
{
   if(event == 0)//differ
	HF_Debug(DEBUG_LEVEL_LOW,"hf ir recv data %X\n",data);
   else if(event == 1)//repeat
	HF_Debug(DEBUG_LEVEL_LOW,"hf ir recv repeat data %X\n",data);
}
...
{	
	...
	if(hfnet_start_ir(HFTHREAD_PRIORITIES_LOW,(hfnet_callback_t)ir_recv_callback)!=HF_SUCCESS)
	{
		HF_Debug(DEBUG_WARN,"start uart fail!\r\n");
	}
