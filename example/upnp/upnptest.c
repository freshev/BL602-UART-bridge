...
USER_FUNC int get_if_ip_addr(void)
{
	char *words[6]={NULL};
	char rsp[64]={0};
	char is_ap_mode=0;
...
USER_FUNC int send_packet(int fd, const void *data, size_t len,struct sockaddr_in *to)
{
	int slen=0;
...
USER_FUNC int send_msearch_rsp_packet(int fd,char *pkt_buffer,int len,uint32_t if_addr,struct sockaddr_in *to)
{
...
USER_FUNC int send_alive_packet(int fd,char *pkt_buffer,int len,uint32_t if_addr)
{
...
USER_FUNC int create_multicast_socket(uint32_t if_addr)
{
	char optval;
	int r;
	int sd;
