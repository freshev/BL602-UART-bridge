...
例子：AT+PWM=4,20000,20		//GPIO4（nReady引脚）输出20KHz占空比20%的PWM波形。
...
static int hf_cmd_pwm(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	...
	if(argc == 3)
	{
		...
		if(strcasecmp(argv[0],"C")==0||strcasecmp(argv[0],"1")==0)//C
		{
			hfgpio_pwm_enable(PWM_C, freq, duty);
		}
		else if(strcasecmp(argv[0],"W")==0||strcasecmp(argv[0],"2")==0)//W
		{
			hfgpio_pwm_enable(PWM_W, freq, duty);
		}
		else if(strcasecmp(argv[0],"R")==0||strcasecmp(argv[0],"3")==0)//R
		{
			hfgpio_pwm_enable(PWM_R, freq, duty);
		}
		else if(strcasecmp(argv[0],"G")==0||strcasecmp(argv[0],"4")==0)//G
		{
			hfgpio_pwm_enable(PWM_G, freq, duty);
		}
		else if(strcasecmp(argv[0],"B")==0||strcasecmp(argv[0],"5")==0)//B
		{
			hfgpio_pwm_enable(PWM_B, freq, duty);
		}
		else
			return -4;
	}
...
{	
	...
	hfgpio_pwm_enable(PWM_C, 20000, 10);
	hfgpio_pwm_enable(PWM_W, 20000, 20);
	hfgpio_pwm_enable(PWM_R, 20000, 30);
	hfgpio_pwm_enable(PWM_G, 20000, 40);
	hfgpio_pwm_enable(PWM_B, 20000, 50);
