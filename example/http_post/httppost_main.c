...
static int hf_httppost_file(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	 int ret=0;
	 
	 if(argc == 2)
	 {
		 ret = http_post_file_thread(argv[0], atoi(argv[1]));
...
static char http_content_data[1024]={0};
static int hf_httppost(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	int ret=0;
	
	if(argc == 2)
	{
		memset(http_content_data,0,1024);
		ret = http_post_thread(argv[0], argv[1], http_content_data);
		if(ret < 0)
			return -1;
		else
		{
			sprintf(rsp, "=%s", http_content_data);
			return 0;
		}
	}
	return -3;
...
{
	{"HTTPPOSTFILE",hf_httppost_file,"   AT+HTTPPOST: HTTPPOST At cmd.\r\n",NULL},
	{"HTTPPOST",hf_httppost,"   AT+HTTPPOST: HTTPPOST At cmd.\r\n",NULL},
...
static int bt_ntf_recv_callback( uint32_t event,uint8_t *data,uint8_t len,uint8_t buf_len)
{
	if(event==HFNET_BT_NTF_DATA_READY)
		HF_Debug(DEBUG_LEVEL_LOW,"ble recv %d bytes %d\n",len,buf_len);
	else if(event==HFNET_BT_NTF_DATA_ENABLE)
		u_printf("ble ntf connected!\n");
	else if(event==HFNET_BT_NTF_DATA_DISENABLE)
		u_printf("ble ntf disconnected!\n");
	
	return len;
}

static int bt_ind_recv_callback( uint32_t event,uint8_t *data,uint8_t len,uint8_t buf_len)
{
	if(event==HFNET_BT_IND_DATA_READY)
		HF_Debug(DEBUG_LEVEL_LOW,"ble ind recv %d bytes %d\n",len,buf_len);
	else if(event==HFNET_BT_IND_DATA_ENABLE)
		u_printf("ble ind connected!\n");
	else if(event==HFNET_BT_IND_DATA_DISENABLE)
		u_printf("ble ind disconnected!\n");
