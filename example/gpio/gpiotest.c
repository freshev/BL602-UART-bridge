...
#define HFGPIO_F_USER_RELOAD			(HFGPIO_F_USER_DEFINE+0)
...
static void USER_FUNC do_user_reload(uint32_t irq, uint32_t fid)
{
	...
	//开发板RELOD脚上拉，如果为高电平说明释放按键
	if(irq == HFPIO_IT_FALL_EDGE)
	{
		press_reload_key=1;
		u_printf("HFPIO_IT_FALL_EDGE\n");
	}
	else if(irq == HFPIO_IT_RISE_EDGE)
	{
		u_printf("HFPIO_IT_RISE_EDGE\n");
	}
	else if(irq == HFPIO_IT_LOW_LEVEL)
	{
		u_printf("HFPIO_IT_LOW_LEVEL\n");
	}
	else if(irq == HFPIO_IT_HIGH_LEVEL)
	{
		u_printf("HFPIO_IT_HIGH_LEVEL\n");
	}
}

static void USER_FUNC test_gpio_start()
{
	if(hfgpio_configure_fpin_interrupt(HFGPIO_F_USER_RELOAD, HFM_IO_TYPE_INPUT|HFPIO_IT_FALL_EDGE, do_user_reload, 1)!=HF_SUCCESS)
	{
