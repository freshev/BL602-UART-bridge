...
#ifdef RDA_AES_CODE
#define RDA_AES_ENC_MODE 1
#define RDA_AES_DEC_MODE 5
                          ...
                          int dir)
{
	...
	int i;
	unsigned int * RK = aes->key;
	for( i = 0; i < 4; i++ ) {
		GET_UINT32_BE( RK[3 - i], key, i << 2);
	}
    	
	if (iv)
       	XMEMCPY((char *)aes->reg, (char *)iv, AES_BLOCK_SIZE);
...
{
	sz = sz/AES_BLOCK_SIZE*AES_BLOCK_SIZE;
	int i;
	unsigned char iv[AES_BLOCK_SIZE];
	XMEMCPY(iv, (char *)aes->reg, AES_BLOCK_SIZE);

	while(sz > 0)
	{
		for(i=0; i<AES_BLOCK_SIZE; i++)
			out[i] = (unsigned char)(in[i]^iv[i]);
		rda_aes_crypt(aes->key, RDA_AES_ENC_MODE, AES_BLOCK_SIZE, NULL, out, out);
		XMEMCPY(iv, out, AES_BLOCK_SIZE);
...
int  AesCbcDecrypt(Aes* aes, byte* out, const byte* in, word32 sz)
{
	...
	while(sz > 0)
	{
		XMEMCPY(temp, in, AES_BLOCK_SIZE);
		rda_aes_crypt(aes->key, RDA_AES_DEC_MODE, AES_BLOCK_SIZE, iv, in, out);
        XMEMCPY(iv, temp, AES_BLOCK_SIZE);
