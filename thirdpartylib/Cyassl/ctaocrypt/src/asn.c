...
#include <cyassl/ctaocrypt/random.h>
    ...
    */
    ...
    #define XGMTIME(c) localtime((c))/*gmtime((c)) modified by Hi-flying L.B. 2015-11-25*/
    #define XVALIDATE_DATE(d, f, t) ValidateDate((d), (f), (t))
#endif
...
int DecodeToKey(DecodedCert* cert, int verify)
{
   ...
   TRACE("%s: Certificate: \n\rSubject: %s\n\rIssuer: %s\n\r\n\r", __FUNCTION__, cert->subject, cert->issuer);
...
int ParseCertRelative(DecodedCert* cert, int type, int verify, void* cm)
{
    ...
    if (badDate != 0)
    {
        CYASSL_MSG("function ParseCertRelative badDate error");
       return badDate;
    }
