...
#endif
#ifdef ECOS_BUILD
	ret = CyaSSL_SendData(ssl, data, sz); 
#else
...
int CyaSSL_Init(void)
{
    ...
    extern void hf_set_cyassl_mem_fun(void);
    hf_set_cyassl_mem_fun();//add by LB from Highflying
...
int CyaSSL_set_tlsext_host_name(CYASSL* ssl, const char* host_name)
{
    int ret;
    CYASSL_ENTER("CyaSSL_set_tlsext_host_name");
    TLSX_Push(&ssl->extensions, TLSX_SERVER_NAME, (void *)host_name, ssl->heap);
    return ret;
}
