...
int TLSX_Push(TLSX** list, TLSX_Type type, void* data, void* heap)
{
...
int TLSX_Write_hostname(byte* output, void *data)
{
    word16 offset = 2; /* list length offset */
    word16 length = (word16)XSTRLEN((char*)data);

    output[offset++] = 0; /* extension type, hostname */

    c16toa(length, output + offset); /* extension length */
    offset += 2;

    XMEMCPY(output + offset, (byte *)data, length); /* extension data */
    offset += length;

    c16toa(offset - 2, output);
    return offset;
}

/** Writes the extensions to be sent into the client hello. */
int TLSX_WriteRequest(CYASSL* ssl, byte* output, byte msgType, word16* pOffset)
{
    int ret = 0;
    word16 offset = 0, extension_offset;
    ...
    while ((extension = list)) {
        ...
        offset += 2; /* extension data length. */

        /* extension data should be written internally. */
        extension_offset = 0;
        switch (extension->type) {
            case TLSX_SERVER_NAME:
                extension_offset = TLSX_Write_hostname(output + offset, extension->data);
                break;
        }
        offset += extension_offset;

        /* writes extension data length. */
        c16toa(extension_offset, output + offset - extension_offset -2);
    }

    if (offset > 2)
        c16toa(offset - 2, output); /* extensions length */
...
int TLSX_GetRequestSize(CYASSL* ssl, byte msgType, word16* pLength)
{
    int ret = 0;
    word16 offset = 0, extension_offset;
    ...
    while ((extension = list)) {
        ...
        switch (extension->type) {
            case TLSX_SERVER_NAME:
                extension_offset =  5 + (word16)XSTRLEN((char*)extension->data);
                break;
        }
        offset += extension_offset;
    }
