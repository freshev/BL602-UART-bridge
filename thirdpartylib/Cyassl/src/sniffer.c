                      ...
                      int typeKey, const char* password, char* error)
{
    ...
    sniffer = (SnifferServer*)os_mem_alloc(sizeof(SnifferServer));
    if (sniffer == NULL) {
        ...
        if (extType[0] == 0x00 && extType[1] == TICKET_EXT_ID) {
            ...
            if (extLen) {
                if (session->ticketID == 0) {
                    session->ticketID = (byte*)os_mem_alloc(ID_LEN);
                    if (session->ticketID == 0) {
                                     ...
                                     char* error)
{
    ...
    /* create a new one */
    session = (SnifferSession*)os_mem_alloc(sizeof(SnifferSession));
    if (session == NULL) {
        SetError(MEMORY_STR, error, NULL, 0);
        return 0;
    }
                                  ...
                                  int* bytesLeft)
{
    ...
    pb = (PacketBuffer*)os_mem_alloc(sizeof(PacketBuffer));
    if (pb == NULL) return NULL;
    
    pb->next  = 0;
    pb->begin = *begin;
    pb->end   = end;
    pb->data = (byte*)os_mem_alloc(added);
