...
/*Proxy-authorization: basic aGVsbG86d29ybGQ= */
	...
	if (wsi->sock < 0) {
...
#endif
		{
			...
			client_addr4.sin_port = htons(WEB_SERVER_PORT);
			client_addr4.sin_len = sizeof(client_addr4);
			client_addr4.sin_addr.s_addr = INADDR_ANY;
		}
	...
	if (context->http_proxy_port) {
		...
		n = send(wsi->sock, context->service_buffer, plen, MSG_DONTWAIT);
		if (n < 0) {
			      ...
			      int ietf_version_or_minus_one)
{
	...
	wsi = (struct libwebsocket *) hfmem_malloc(sizeof(struct libwebsocket));
	if (wsi == NULL)
		goto bail;
