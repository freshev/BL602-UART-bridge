		...
		struct libwebsocket *wsi, struct libwebsocket_pollfd *pollfd)
{
	...
	case LWS_CONNMODE_WS_CLIENT_ISSUE_HANDSHAKE:
		...
		if (wsi->use_ssl && !wsi->ssl) {
			wsi->ssl = CyaSSL_new(context->ssl_client_ctx);
			CyaSSL_set_fd(wsi->ssl, wsi->sock);
		}

		if (wsi->use_ssl) {
			lws_latency_pre(context, wsi);
			msleep(100);
			n = CyaSSL_connect(wsi->ssl);
			...
			if (n < 0) {
				...
				lwsl_notice("SSL_connect success\n");
			if (n <= 0) {
				/*
		...
		while (ext && ext->callback) {
			...
			wsi->active_extensions_user[
				wsi->count_active_extensions] =
					 hfmem_malloc(ext->per_session_data_size);
...
check_accept:
	...
	wsi->u.ws.rx_user_buffer = (char *)hfmem_malloc(n);
		...
		struct libwebsocket *wsi, char *pkt)
{
	...
	if (n != 16) {
		lwsl_err("Unable to get random number.\n");
		libwebsocket_close_and_free_session(context, wsi,
					     LWS_CLOSE_STATUS_NOSTATUS);
		return NULL;
	}
...
#if 1  //for HF server compatible test
