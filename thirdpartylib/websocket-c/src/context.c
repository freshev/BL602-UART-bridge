...
libwebsocket_create_context(struct lws_context_creation_info *info)
{
	...
	context = (struct libwebsocket_context *)
				hfmem_malloc(sizeof(struct libwebsocket_context));
	if (!context) {
...
*/
	...
	context->max_fds = 24;/*MAX socket can be opened, should use macro*/
	context->fds = (struct libwebsocket_pollfd *)hfmem_malloc(sizeof(struct libwebsocket_pollfd) *
							      context->max_fds);
	if (context->fds == NULL) {
	...
	context->lws_lookup = (struct libwebsocket **)
		      hfmem_malloc(sizeof(struct libwebsocket *) * context->max_fds);
	if (context->lws_lookup == NULL) {
		lwsl_err(
		  "Unable to allocate lws_lookup array for %d connections\n",
							      context->max_fds);
		hfmem_free(context->fds);
		hfmem_free(context);
		return NULL;
	}
