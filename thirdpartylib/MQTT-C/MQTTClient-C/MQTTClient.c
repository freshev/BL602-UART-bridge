...
char recv_puback_qos1=0;
char recv_puback_qos2=0;
...
int sendPacket(Client* c, int length, Timer_mqtt* timer)
{
    int rc = FAILURE, 
        sent = 0;
...
int readPacket(Client* c, Timer_mqtt* timer) 
{
    int rc = 0;
    MQTTHeader header = {0};
    int len = 0;
...
#else
    //Liubo
    int recv_num = c->ipstack->mqttread(c->ipstack, c->readbuf, 1, left_ms(timer));
    if(recv_num < 0)
        return -1;
    if(recv_num > 0)
		ping_outstanding = 0; 
...
int deliverMessage(Client* c, MQTTString* topicName, MQTTMessage* message)
{
    ...
    for (i = 0; i < MAX_MESSAGE_HANDLERS; ++i)
    {  
		//u_printf("messageHandlers[%d] topicFilter:%s\n",i,c->messageHandlers[i].topicFilter);
...
int cycle(Client* c, Timer_mqtt* timer)
{
    // read the socket, see what work is due
    //unsigned short packet_type = readPacket(c, timer);
    int packet_type = readPacket(c, timer);// Rui: unsigned short (-1)0xFFFF --> int0x0000FFFF
    if(packet_type<0)
        return FAILURE;
    ...
    switch (packet_type)
    {
		...
		case PUBREL:
		{
			unsigned short msg_id = c->readbuf[2]*256 + c->readbuf[3];
			len = MQTTSerialize_ack(c->buf, c->buf_size, PUBCOMP, 0, msg_id);
			if (len <= 0)
 				rc = FAILURE;
			else
			{
...
{   
    ...
    while (!expired(&timer))
    {   
		//Rui: in order to get MQTT PINGRESP ret
		#if 0
    ...
    return rc;
#else
    static unsigned int keepalive_send_time = 0;
    if ((c->keepAliveInterval != 0) && (c->ping_outstanding == 1))
    {
    	unsigned int now = hfsys_get_time();
        if (keepalive_send_time == 0)
			keepalive_send_time = now;
		
        if ((unsigned int)(now - keepalive_send_time) > (g_hf_config_file.mqtt_heart_time*15/10)*1000)
        {
        	u_printf("heart time timeout MQTT closed\r\n");
        	return FAILURE;
        }
    }
    else
        keepalive_send_time = 0;
    return SUCCESS;
#endif
}
...
int waitfor(Client* c, int packet_type, Timer_mqtt* timer)
{
    int rc = 0;
...
{  
    int rc = FAILURE;
    Timer_mqtt timer;     // we might wait for incomplete incoming publishes to complete
