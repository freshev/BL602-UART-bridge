...
#define JLP_UUID "F4AA74" 
#define IDT_CLOUD_PUB_KEY "0360BB51E8505F8DBDF0E1EA1713D1BCDB47B82F23FBA42BA3C50CC3845908F83B"
...
#define USER_LINK		(HFGPIO_F_USER_DEFINE+0)
#define USER_READY		(HFGPIO_F_USER_DEFINE+1)
#define USER_RELOAD		(HFGPIO_F_USER_DEFINE+2)

#define JDCONFIG_OFFSET (0)
#define JD_SOFTAP_ADDR  (1024)
#define JD_MAGIC_ADDR  	(1024+10)
#define JD_PRIVATE_ADDR (JD_MAGIC_ADDR+10)

#define JD_CONFIG_TIMEOUT (1800)//seconed
#define JD_CONFIG_NET_TIMEOUT (60)//seconed

typedef struct _user_dev_status_t {
	uint8_t OnOff_state;
} user_dev_status_t;

typedef struct _https_post_data
{
	int  buflen;
	char revbuf[1024];
}https_post_data;
...
extern HF_JD_CONFIG g_hf_jdconfig;
extern char dhcp_ok_flag;
extern JLPInfo_t jdArgs;
extern https_post_data https_post_data_t;
extern int has_softap_flag;
extern char scan_suc;
...
/**
 * @brief: 断开与云端的连接TCP常连接，释放资源，设备断电休眠前，需要调用。
 *
 * @returns:
 */
int joylink_server_st_close(void);
 ...
 */
int joylink_dev_get_snapshot(char *out_snap, int32_t out_max);
 ...
 * @returns: E_RET_OK 成功, E_RET_ERROR 发生错误
 */
E_JLRetCode_t joylink_dev_report_status_immediately(char *mac_str, char *data, int len);
 ...
 *
 *
 * @returns: 1, 已绑定；0, 未绑定；-1, 检测出错； -2, feedid为空。
 */
int joylink_dev_check_bind_status(void);

/**
 * @brief: 获取设备激活绑定时设备所处位置的天气信息
 *
 *
 * @returns: NULL, 查询失败；其它, 天气信息，内存需调用者释放。
 */
char *joylink_get_weather_info_by_ip(int number, char **service);

/***************************************************************
功   能：获取设备所在城市的天气信息
参   数：
    city : 所在城市名称，必须是城市名，无需带上省份。
            如"安庆"、"安庆市"均可, 请不要传"安徽安庆"。
    number : 服务类型数
    service : 服务类型
返回值 ：
    NULL, 查询失败；其它, 天气信息，内存需调用者释放。
***************************************************************/
char *joylink_get_weather_info_by_cityname(char *city, int number, char **service);
