#ifndef __JOYLINK_BLE_SDK_H__
#define __JOYLINK_BLE_SDK_H__
...
typedef enum
{
	...
	E_JL_NET_CONF_ST_WIFI_RSSI					= 0x0E, //WIFI路由器的RSSI，data占用两个字节，int16_t类型。
	E_JL_NET_CONF_ST_WIFI_ERROR_DESC			= 0x0F, //连接WIFI失败后，从底层芯片读取到的错误信息，data是字符串格式，len为data长度。厂商需将该字段内容与底层错误一一对应。
	E_JL_NET_CONF_ST_CONFIG_FAILED_INFO 		= 0x10, //连接WIFI失败后，从底层芯片底层返回的状态码和路由器的RSSI组成的字符串一并上报，data是字符串格式，len为data长度。
}E_JL_NET_CONF_ST;
...
Calls: SDK内部函数
Called By: 配网状态改变之后
Input: @st: 配网状态,见 E_JL_NET_CONF_ST
            JL_BLE_SDK_VERSION为2.0.9以上的版本，0E和0F合并为10上报，
              注意:配网失败时状态码10须作为最后状态码，必报，开发者需尽可能收集底层返回信息，方便失败原因排查
              (底层未返回信息时传默认值255，扫描不到ssid时传默认值0dBm)。
              例如,配网失败原因为密码错误时，底层返回WLAN_FW_4WAY_HANDSHAKE_ERROR_PSK_TIMEOUT_FAILURE(对应状态码8), 此时路由器rssi为-45dBm，
              则data指向"08_-45dBm"（参数中间用'_'隔开），data_len为字符串长度9。 
Input: @data: 相应st对应的数据
Output: None
Return: 0：成功
       -1：失败
Others: None
