...
{
	if(g_hf_config_file.uart1_debug != 1 || g_hf_config_file.debug_level == 0)
		return 0;
    int32_t ret = 0;
	va_list ap;
	int len;
	...
	dbg_buf = (char *)malloc(DBG_BUFF_LEN);
	if(dbg_buf==NULL)
		return 0;
	va_start(ap,fmt);
	memset(dbg_buf, 0, DBG_BUFF_LEN);
	vsnprintf(dbg_buf,DBG_BUFF_LEN-1, fmt, ap);  //change by lqj 2015.07.16
	va_end(ap);
	len = strlen(dbg_buf);
	if(len<DBG_BUFF_LEN -5&&len>=2)
	{
		if(dbg_buf[len-1]==0x0A&&dbg_buf[len-2]!=0x0D)
		{
			dbg_buf[len-1]=0x0D;
			dbg_buf[len]=0x0A;
			dbg_buf[len+1]=0;
	...
	if(g_hf_config_file.uart1_debug==1)
	{
		u_printf("%s", dbg_buf);
	}
//	else
//		ret = hfuart_send(HFUART0, dbg_buf, strlen(dbg_buf),1000);
...
int32_t jl_platform_sprintf(char *str, const char *fmt, ...)
{
    ...
    va_list args;
    va_start(args, fmt);
    ret = vsnprintf(str, 1399, fmt, args);
    va_end(args);
    return ret;
}
