...
hfthread_mutex_t sock_send_recv_mutex = NULL;
...
{
	int ret;
	hfthread_mutext_lock(sock_send_recv_mutex);
	ret = send(fd, buf, len, flags);
	u_printf("jl_platform_send %d\r\n",ret);
	hfthread_mutext_unlock(sock_send_recv_mutex);
	return ret;
}
...
{
	u_printf("jl_platform_sendto fd %d len %d\r\n",fd,len);
    struct sockaddr addr;
...
{
	u_printf("jl_platform_recvfrom fd %d len %d\r\n",fd,len);
    int32_t ret;
    struct sockaddr addr;
