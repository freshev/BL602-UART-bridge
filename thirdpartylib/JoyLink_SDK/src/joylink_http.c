...
int https_auth_start(char *url, const char *sendbuf, int sendnum);

// #define __JL_PLATFORM_HTTP_DEBUG__
...
int32_t jl_platform_https_request(jl_http_t *info)
{
	...
	https_post_data_t.buflen = https_auth_start(host,request,info->resp_len);

	if(https_post_data_t.buflen <=0)
		goto RET;
	
	u_printf("\r\n**********************  https_auth_start  **********************\r\n");

	memcpy(info->response,https_post_data_t.revbuf,https_post_data_t.buflen);

	log_info("... read data length = %d, response data = \r\n%s\r\n", https_post_data_t.buflen, info->response);
	ret = joylink_dev_http_parse_content(info->response, https_post_data_t.buflen, info->response, info->resp_len);
	
	u_printf("\r\n**********************  joylink_dev_http_parse_content  %d	**********************\r\n",ret);
RET:
	if(request)
