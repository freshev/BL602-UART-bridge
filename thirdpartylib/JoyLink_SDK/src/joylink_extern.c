...
char front_st = 0;
char cloud_disconnect_flag=0;
JLPInfo_t jdArgs;
https_post_data https_post_data_t;
...
//		jl_send_net_config_state(E_JL_NET_CONF_ST_CLOUD_CONNECT_SUCCEED,NULL,0);
	...
	if((st == 2) && (front_st == 1 || front_st == 0))
	{	
		cloud_disconnect_flag = 0;
		front_st = st;
	}
	else if((st != 2) && (front_st == 2))
	{	
		cloud_disconnect_flag = 1;
		front_st = st;
	}
...
{
	u_printf("***********joylink_server_notify_auth_ok***********\n");
	return 1;
}
...
{
	char *feedid=NULL;
	uint8_t feedid_data[100]={0};
	log_info("message = %s", message);
	
	feedid = strstr(message,"feed_id");
	memcpy(feedid_data,feedid+10,18);
	log_info("len = %d feedid = %s",18,feedid_data);
//	if(has_softap_flag>0)
	//	jl_send_net_config_state(E_JL_NET_CONF_ST_IOT_CONNECT_SUCCEED,feedid_data,18); 
	return E_RET_OK;
}
...
joylink_dev_set_attr_jlp(JLPInfo_t *jlp)
{
	...
	*/
	...
	u_printf("\r\n=======save Joylink Parameters=======\r\n");

	hffile_userbin_write(JDCONFIG_OFFSET, (char*)jlp, sizeof(JLPInfo_t));
...
{
	jl_platform_strcpy(out, (char *)g_hf_jdconfig.jd_mac);
	return E_RET_OK;
}
...
{
	u_printf("joylink_dev_get_private_key %s\r\n",g_hf_jdconfig.jd_prikey);
	jl_platform_strcpy(out, g_hf_jdconfig.jd_prikey);
	return E_RET_OK;
...
{	
	...
	*/
	...
	u_printf("=======get Joylink Parameters=======\r\n");
	...
	hffile_userbin_read(JDCONFIG_OFFSET, (char*)&jdArgs, sizeof(JLPInfo_t));

	jl_platform_strcpy(jlp->feedid, jdArgs.feedid);
	jl_platform_strcpy(jlp->accesskey, jdArgs.accesskey);
	jl_platform_strcpy(jlp->localkey, jdArgs.localkey);
	jl_platform_strcpy(jlp->joylink_server, jdArgs.joylink_server);
	jlp->server_port = jdArgs.server_port;
	jl_platform_strcpy(jlp->domain, jdArgs.domain);
	jl_platform_strcpy(jlp->token, jdArgs.token);
	
	joylink_dev_get_user_mac(jlp->mac);
	joylink_dev_get_private_key(jlp->prikey);

	jlp->is_actived = jdArgs.is_actived;
	jlp->model_code_flag = jdArgs.model_code_flag;
 ...
 */
int
joylink_dev_get_snapshot_with_retcode(int32_t ret_code, char *out_snap, int32_t out_max)
{
...
int
joylink_dev_get_snapshot(char *out_snap, int32_t out_max)
{
	return joylink_dev_get_snapshot_with_retcode(0, out_snap, out_max); 
}
 ...
 * @brief: 设备状态改变，立即上报。
 *
 * @param[in] mac_str: 设备mac地址，普通设备mac可为空，子设备时必须有效
 * @param[in] data: 上报数据内容
 * @param[in] len: 上报数据长度
 *
 * @returns: E_RET_OK 成功, E_RET_ERROR 发生错误
 */
E_JLRetCode_t
joylink_dev_report_status_immediately(char *mac_str, char *data, int len)
{
	if(JLP_DEV_TYPE == E_JLDEV_TYPE_NORMAL){
		return joylink_server_event_req(data, len);
	}
	if(JLP_DEV_TYPE == E_JLDEV_TYPE_GW){
		return joylink_server_subdev_event_req(mac_str, data, len);
	}
}
...
{
	u_printf("/*******joylink_dev_ota_status_upload********/\r\n");
	return;	
}
...
/**
 * @brief: 解绑通知，用户解绑设备之后，SDK调用此接口通知外部（子设备除外）。
 *
 * @returns: 
 */
 
extern char enter_smtble_flag;
void joylink_dev_unbind_notify(void)
{
	u_printf("joylink_dev_unbind_notify\r\n");
	enter_smtble_flag = 1;
}

#if defined _JOYLINK_UART_PROCOTOL_SUPPORTED_
/**
 * @brief :获取平台产品类型, 自定义接入/标品接入
 * 
 * @return  0:自定义接入;1:标品接入.
 */
int joylink_dev_get_product_type(void)
{
	int stdFlag = 1;
	return stdFlag;
}
