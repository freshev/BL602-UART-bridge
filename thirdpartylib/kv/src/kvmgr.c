...
typedef struct _kv_mgr_t
{
    ...
    hfthread_sem_t gc_sem;
    hfthread_mutex_t kv_mutex;
...
{
    if(hfuflash_read(KV_FLASH_ADDRESS+offset, (char *)buf, nbytes) == nbytes)
        return RES_OK;
    else
        return -1;
}
...
{
    if(hfuflash_write(KV_FLASH_ADDRESS+offset, (char *)buf, nbytes) == nbytes)
        return RES_OK;
    else
        return -1;
}
...
{
    return hfuflash_erase_page(KV_FLASH_ADDRESS+offset, size/4096);
}
...
static void trigger_gc(void)
{
    ...
    hfthread_create(kv_module_gc, "kv-gc", KV_GC_STACK_SIZE, NULL, HFTHREAD_PRIORITIES_LOW, NULL, NULL);
}
...
static int kv_item_del(kv_item_t *item, int mode)
{
    ...
    else if (mode == KV_ORIG_REMOVE)
    {
        ...
        origin_key = (char *)hfmem_malloc(hdr.key_len);
        if (!origin_key)
        {
            return RES_MALLOC_FAILED;
        }
        new_key = (char *)hfmem_malloc(hdr.key_len);
        if (!new_key)
        {
            hfmem_free(origin_key);
            return RES_MALLOC_FAILED;
        }
...
{
    char *p = (char *)hfmem_malloc(item->len);
    if (!p)
    {
        return RES_MALLOC_FAILED;
    }
...
static int __item_find_cb(kv_item_t *item, const char *key)
{
    ...
    item->store = (char *)hfmem_malloc(item->hdr.key_len + item->hdr.val_len);
    if (!item->store)
    {
        return RES_MALLOC_FAILED;
    }
...
static int __item_del_by_prefix_cb(kv_item_t *item, const char *prefix)
{
    ...
    key = (char *)hfmem_malloc(item->hdr.key_len + 1);
    if (!key)
        return RES_MALLOC_FAILED;
...
static kv_item_t *kv_item_traverse(item_func func, uint8_t blk_index, const char *key)
{
    ...
    do
    {
        item = (kv_item_t *)hfmem_malloc(sizeof(kv_item_t));
        if (!item)
        {
            return NULL;
        }
...
static int kv_item_store(const char *key, const void *val, int len, uint16_t origin_off)
{
    ...
    store.p = (char *)hfmem_malloc(store.len);
    if (!store.p)
    {
        return RES_MALLOC_FAILED;
    }
...
void kv_module_gc(void *arg)
{
    ...
    if (hfthread_mutext_lock(g_kv_mgr.kv_mutex) != 0)
    {
        goto exit;
    }
...
exit:
    ...
    hfthread_mutext_unlock(g_kv_mgr.kv_mutex);
    if (g_kv_mgr.gc_waiter > 0)
    {
        hfthread_sem_signal(g_kv_mgr.gc_sem);
    }
...
int kv_module_del(const char *key)
{
    kv_item_t *item;
    int ret;
    if ((ret = hfthread_mutext_lock(g_kv_mgr.kv_mutex)) != RES_OK)
    {
        return ret;
    }
...
int kv_module_set(const char *key, const void *val, int len, int sync)
{
    kv_item_t *item;
    int ret;
    ...
    {
        (g_kv_mgr.gc_waiter)++;
        hfthread_sem_wait(g_kv_mgr.gc_sem, 0xffffffffu);
    }
...
int kv_module_init(void)
{
    ...
    if ((ret = hfthread_mutext_new(&(g_kv_mgr.kv_mutex))) != 0)
    {
        return ret;
    }
    ...
    if ((ret = hfthread_sem_new(&(g_kv_mgr.gc_sem), 0)) != RES_OK)
    {
        return ret;
    }
...
void kv_module_deinit(void)
{
    ...
    hfthread_sem_free(g_kv_mgr.gc_sem);
    hfthread_mutext_free(g_kv_mgr.kv_mutex);
}
