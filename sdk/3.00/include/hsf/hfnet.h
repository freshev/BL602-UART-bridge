...
void HSF_IAPI hfnet_lock(void);

void HSF_IAPI hfnet_unlock(void);
...
int HSF_IAPI hfnet_packet_encode(char *src, int srclen, char *dst, int dstlen);
 ...
 *
 * @param[in] rsp: data you want send
 *			 len: the size of rsp
 ...
 * @note None.
 */
int HSF_API hfnet_assis_write(char *rsp, int len, uint32_t addr, uint16_t port);

/**
 * @brief start UART0(set by AT+UART), and register callback function which will be called when system happen the follow cases.
 * ==========================================
 * | HFNET_UART0_DATA_READY                 |
 * ==========================================
 *
 ...
 * ==========================================
 *
 * @param[in] uart_id: 0 or 1
 *			 uxpriority: priority, default is 'HFTHREAD_PRIORITIES_LOW',  it's better not to modify it
 *			 p_callback: a function pointer, called back when recv data from UART0
 ...
 * @note None.
 */
int HSF_API hfnet_start_uart_ex(int uart_id, uint32_t uxpriority,hfnet_callback_t p_callback, int stack_size);

/**
 * @brief start SOCKETA(set by AT+NETP), and register callback function which will be called when system happen the follow cases.
 * ==========================================
...
/**
 * @brief start SOCKETB(set by AT+SOCKB), and register callback function which will be called when system happen the follow cases.
 * ==========================================
 * | HFNET_SOCKETB_DATA_READY               |
 * |----------------------------------------|
 * | HFNET_SOCKETB_CONNECTED                |
...
/**
 * @brief check whether or not socketA is connected if in TCP mode.
 *
 * @param[in] None
 * @return[out] 0-not connected, 1-have connected
 * @see None.
 * @note None.
 */
int HSF_API hfnet_socketa_is_connected(void);
 ...
 * @note None.
 */
int HSF_API hfnet_socketa_get_client(int cid, hfnet_socketa_client_t *p_client);
...
/**
 * @brief check whether or not socketB is connected if in TCP mode.
 *
 * @param[in] None
 * @return[out] 0-not connected, 1-have connected
 * @see None.
 * @note None.
 */
int HSF_API hfnet_socketb_is_connected(void);
...
/**
 * @brief get module's WAN ip address offered by DHCP Server or set by AT+WANN.
 *
 ...
 * @note None.
 */
int HSF_API hfnet_get_lan_ip(uint32_t * ip, uint32_t *mask);
...
#ifdef BLE_FUNCTION
/**
 * @brief start ble notification and register callback function which will be called when system happen the follow cases.
 * ==========================================
 * | HFNET_BT_NTF_DATA_READY                |
 * |----------------------------------------|
 * | HFNET_BT_NTF_DATA_ACK                  |
 * |----------------------------------------|
 ...
 * @note None.
 */
int HSF_API hfnet_start_ble_ntf(uint32_t uxpriority, hfble_callback_t p_callback);

/**
 * @brief start ble indicate and register callback function which will be called when system happen the follow cases.
 * ==========================================
 * | HFNET_BT_IND_DATA_READY                |
 * |----------------------------------------|
 * | HFNET_BT_IND_DATA_ACK                  |
 * |----------------------------------------|
 ...
 * @note None.
 */
int HSF_API hfnet_start_ble_ind(uint32_t uxpriority, hfble_callback_t p_callback);

/**
 * @brief send data via ble notification.
 *
 * @param[in] data: the data buffer you want send
 *			 len: the size of send buffer
 *			 timeouts: no use
 * @return[out] the size send out by ble notification
 * @see None.
 * @note None.
 */
int HSF_API hfnet_bt_ntf_send(char *data, uint32_t len, uint32_t timeous);

/**
 * @brief send data via ble indicate.
 *
 * @param[in] data: the data buffer you want send
 *			 len: the size of send buffer
 *			 timeouts: no use
 * @return[out] the size send out by ble indicate
 * @see None.
 * @note None.
 */
int HSF_API hfnet_bt_ind_send(char *data, uint32_t len, uint32_t timeous);
#endif

/**
 * @brief start infrared notification and register callback function which will be called when system happen the follow cases.
 * ==========================================
 * | HFNET_BT_NTF_DATA_READY                |
 * |----------------------------------------|
 * | HFNET_BT_NTF_DATA_ACK                  |
 ...
 * @note None.
 */
int HSF_API hfnet_start_ir(uint32_t uxpriority,hfnet_callback_t p_callback);
