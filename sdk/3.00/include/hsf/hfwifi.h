...
void HSF_IAPI hfwifi_stack_init(void);

int HSF_IAPI hfwifi_get_mode(void);

int HSF_IAPI hfwifi_get_state(void);
...
int HSF_IAPI hfwifi_write_sta_mac_address_product(uint8_t *mac, char *key);
...
/**
 * @brief launch a wifi scan operation. scan from ch1 to ch14, takes about 3.4 seconds to finish.
 * 		Max 50 items, then call callback function to deal with these items in applicaiton level
 * 
 ...
 *			 scan_time: totol scan timeout time, in seconds, default is 3s
 * @return[out] the number of wifi
 * @see None.
 * @note None.
 */
 ...
 *
 * @param[in] mac: deauth mac, NULL means delete all stations
 * @return[out] the number of deauthed station
 * @see None.
 * @note None.
 */
 ...
 *			 len: the size of data buffer, <480
 ...
 *
 * @param[in] None
 * @return[out] disconnect reason code
 * @see None.
 * @note this function will reboot.
*/
unsigned short hfwifi_get_disconnect_code(void);
